...
| PE-46 | 将来拡張: Claude Code スラッシュコマンド | 現在 `/` はユーザー定義テンプレのみ。将来 Claude Code スラッシュコマンドを統合する際は `SlashSuggest` の候補に kind 別で混在表示（`SlashSuggestItem = { kind: 'template' \| 'command' }`）する方針 |
...
- `src/components/PromptPalette/SlashSuggest.tsx` — `/` インラインサジェスト（v1.1）
...
- `src/lib/templateApply.ts` — `applyTemplateBodyToDraft`（流し込み + プレースホルダ選択状態化、v1.1）
...
- Claude Code スラッシュコマンド補完（将来 `SlashSuggest` を拡張して統合予定）
